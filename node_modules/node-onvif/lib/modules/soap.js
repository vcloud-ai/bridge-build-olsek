...
OnvifSoap.prototype._request = function(oxaddr, soap) {
	const isHttps = oxaddr.protocol.includes('https');
	...
	let promise = new Promise((resolve, reject) => {
		...
		// console.log(post_opts);

		if (isHttps) post_opts.agent = new https.Agent({ rejectUnauthorized: false });

		const requestHandler = isHttps ? mHttps : mHttp;
		let req = requestHandler.request(post_opts, (res) => {
