...
OnvifDevice.prototype.getUdpStreamUrl = function () {
  if (!this.current_profile) return { streams: [] };

  // console.log('CURRENT PROFILE ====> ' + JSON.stringify(this.current_profile, null, 2));
  // console.log('PROFILE LIST ====> ' + JSON.stringify(this.profile_list, null, 2));

  const streams = this.profile_list.map((profile, index) => {
    const streamData = {};
    const encoder = profile["video"]["encoder"] || {};
    streamData.url = profile["stream"]["udp"] || '';
    streamData.isMainStream = !index;

    const ipAddressOrHost = new URL(this.xaddr).hostname;
    let urlIpAddress = null;

    // console.log('NODE_ONVIF === ADD_ONVIF_CAMERA === 1', { ipAddressOrHost, urlIpAddress });
    const urlIpAddressArr = streamData.url.match(/\/\/([^:/]+)/);
    // console.log('NODE_ONVIF === ADD_ONVIF_CAMERA === 2', { urlIpAddressArr });
    if (urlIpAddressArr && urlIpAddressArr.length) urlIpAddress = urlIpAddressArr[1];
    if (urlIpAddress) streamData.url = streamData.url.replace(urlIpAddress, ipAddressOrHost);
    // console.log('NODE_ONVIF === ADD_ONVIF_CAMERA === 3', { finalUrl: streamData.url });
  ...
  const info = streams.length ? _.uniqBy(streams, 'url').slice(0, 2) : [] ;
  return info;
};
...
OnvifDevice.prototype.init = function (callback) {
  let promise = new Promise((resolve, reject) => {
    this._getSystemDateAndTime()
      .then(() => {
        // console.log('_getSystemDateAndTime');
        return this._getCapabilities();
      })
      .then(() => {
        // console.log('_getCapabilities');
        return this._getDeviceInformation();
      })
      .then(() => {
        // console.log('_getDeviceInformation');
        return this._mediaGetProfiles();
      })
      .then(() => {
        // console.log('_mediaGetProfiles');
        return this._mediaGetStreamURI();
      })
      .then(() => {
        // console.log('_mediaGetStreamURI');
        return this._mediaGetSnapshotUri();
      })
      .then(() => {
        // console.log('_mediaGetSnapshotUri');
