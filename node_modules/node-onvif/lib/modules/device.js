...
//         console.log(this.profile_list);
  ...
  if (!this.current_profile) return [];
	...
	// console.log('CURRENT PROFILE ====> ' + JSON.stringify(this.current_profile));
	// console.log('PROFILE LIST ====> ' + JSON.stringify(this.profile_list));
	
  const streams = this.profile_list.map(profile => {
  const url = profile["stream"]["udp"];
  const codec = profile["video"]["encoder"] ? profile["video"]["encoder"]["encoding"] : 'H265';
  return { url, codec };
});
...
OnvifDevice.prototype._getCapabilities = function () {
  let promise = new Promise((resolve, reject) => {
    this.services.device.getCapabilities((error, result) => {
      ...
      if (error) {
        reject(
          new Error("Failed to initialize the device: " + error.toString())
        );
        return;
      }
...
//       console.log("==== mediaGetProfiles ====");
      // console.log(result.data.GetProfilesResponse.Profiles);
//       console.log("==========================");
      ...
      profiles.forEach((p) => {
        ...
        if (p["VideoSourceConfiguration"]) {
          profile["video"]["source"] = {
            ...
            bounds: {
              width: parseInt(
                p["VideoSourceConfiguration"]["Bounds"]["$"]["width"],
                10
              ),
              height: parseInt(
                p["VideoSourceConfiguration"]["Bounds"]["$"]["height"],
                10
              ),
              x: parseInt(
                p["VideoSourceConfiguration"]["Bounds"]["$"]["x"],
                10
              ),
              y: parseInt(
                p["VideoSourceConfiguration"]["Bounds"]["$"]["y"],
                10
              ),
            },
          };
        ...
        if (p["AudioEncoderConfiguration"]) {
          profile["audio"]["encoder"] = {
            ...
            samplerate: parseInt(
              p["AudioEncoderConfiguration"]["SampleRate"],
              10
            ),
      ...
      if (profile) {
        ...
        if (protocol) {
          ...
          this.services.media.getStreamUri(params, (error, result) => {
            ...
            if (!error) {
              let uri =
                result["data"]["GetStreamUriResponse"]["MediaUri"]["Uri"];
      ...
      if (profile) {
        ...
        this.services.media.getSnapshotUri(params, (error, result) => {
          ...
          if (!error) {
            try {
              let snapshotUri =
                result["data"]["GetSnapshotUriResponse"]["MediaUri"]["Uri"];
